...
namespace Discount.API.Repositories
{
    ...
    public class DiscountRepository : IDiscountRepository
    {
        ...
        public async Task<bool> CreateDiscount(Coupon coupon)
        {
            ...
            var res = await connection.ExecuteAsync(commadnText, new { ProductName = coupon.ProductName, Description = coupon.Description, Amount = coupon.Amount });
            if (res > 0)
                return true;
            return false;
        }
        ...
        public async Task<bool> UpdateDiscount(Coupon coupon)
        {
            ...
            var res = await connection.ExecuteAsync(commadnText, new { ID=coupon.ID,  ProductName = coupon.ProductName, Description = coupon.Description, Amount = coupon.Amount });
            if (res > 0)
                return true;
            return false;
        }
